...
if(isset($_SESSION['user'])){
    ...
    try{
        ...
        foreach($stmt as $row){
            ...
            $output .= "
                <tr>
                    <td><input type='checkbox' class='checkbox' value='".$row['cartid']."'></td>
                    ...
                    <td class='input-group'>
                        ...
                        <input type='text' class='form-control quantity' value='".$row['quantity']."' id='qty_".$row['cartid']."'>
                        <span class='input-group-btn'>
                            <button type='button' class='btn btn-default btn-flat add' data-id='".$row['cartid']."'><i class='fa fa-plus'></i></button>
                        </span>
                    </td>
                    <td> Rp. ".number_format($subtotal,0,',','.')."</td>
                    <td><button type='button' class='btn btn-danger btn-flat cart_delete' data-id='".$row['cartid']."'><i class='fa fa-remove'></i></button></td>
                </tr>
            ";
        }
        $output .= "
    ...
    if(count($_SESSION['cart']) != 0){
        ...
        foreach($_SESSION['cart'] as $row){
            ...
            $output .= "
                <tr>
                    <td><button type='button' class='btn btn-danger btn-flat cart_delete' data-id='".$row['productid']."'><i class='fa fa-remove'></i></button></td>
                    <td><img src='".$image."' width='30px' height='30px'></td>
                    <td>".$product['name']."</td>
                    ...
                    <td class='input-group'>
                        ...
                        <input type='text' class='form-control quantity' value='".$row['quantity']."' id='qty_".$row['productid']."'>
                        <span class='input-group-btn'>
